<div align="center">

# Yeison Zaraza · AryeiCode
### Fullstack Developer · .NET & React · Microservicios · Cloud Azure

*Construyo sistemas donde el front y el back funcionan juntos para generar impacto real.*

[![LinkedIn](https://img.shields.io/badge/LinkedIn-yeison--zaraza-0077B5?style=flat-square&logo=linkedin)](https://linkedin.com/in/yeisonzaraza)
[![Email](https://img.shields.io/badge/Email-yeison.zaraza.m@gmail.com-D14836?style=flat-square&logo=gmail)](mailto:yeison.zaraza.m@gmail.com)
[![Location](https://img.shields.io/badge/📍-Manizales%2C_Colombia-green?style=flat-square)](https://github.com/AryeiCode)

</div>

---

## Sobre mí

Soy Ingeniero de Sistemas con más de 4 años de experiencia construyendo aplicaciones empresariales escalables. He trabajado en sectores como energía y tecnología corporativa, participando en todo el ciclo de desarrollo — desde el levantamiento de requerimientos hasta el despliegue en producción.

Me motiva entender el negocio detrás del código: saber por qué existe una funcionalidad y qué problema real resuelve para el usuario final.

- 🔧 **Backend:** .NET, C#, ASP.NET Core, Microservicios
- 🎨 **Frontend:** Angular, React, TypeScript
- ☁️ **Cloud (experiencia práctica):** Microsoft Azure, Docker, Kubernetes (AKS)
- 📐 **Patrones aplicados en proyectos:** Clean Architecture, DDD, CQRS, SOLID
- 🤖 **Desarrollo asistido por IA:** GitHub Copilot, Claude

---

## Experiencia destacada

### 🏢 Global MVM · Analista de Soluciones de Software
`Ago 2023 – Ene 2026`
- Implementé un flujo de desarrollo asistido por IA con GitHub Copilot y Claude para generación y optimización de pruebas, logrando un incremento del **80% en cobertura** y reduciendo el tiempo de desarrollo de tests en más del **60%**
- Reduje en un **80% la deuda técnica** identificada por SonarQube mediante refactorización y buenas prácticas
- Lideré la migración del ambiente de calidad a un entorno de ciberseguridad, mejorando la postura de seguridad y el cumplimiento normativo

### 🏢 Newsoft · Consultor Técnico TI / Desarrollador Full Stack
`Mar 2021 – Jun 2023`
- Diseñé e implementé arquitecturas escalables con .NET, Angular y React para más de **5.000 usuarios activos mensuales**
- Reduje el tiempo de despliegue en un **25%** mediante Clean Architecture y CI/CD
- Implementé APIs REST con autenticación JWT, Entity Framework Core y SQL Server

### 🏢 Accenture · Desarrollador Mobile
`Ago 2019 – Jul 2020`
- Desarrollé aplicaciones móviles con React Native integrando tecnología Bluetooth beacons
- Integré Azure Cosmos DB y SQL Server en arquitecturas cloud

---

## Stack tecnológico

**Backend**

![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=c-sharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=flat-square&logo=dotnet&logoColor=white)
![ASP.NET Core](https://img.shields.io/badge/ASP.NET_Core-512BD4?style=flat-square&logo=dotnet&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=spring-boot&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white)

**Frontend**

![Angular](https://img.shields.io/badge/Angular-DD0031?style=flat-square&logo=angular&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat-square&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)

**Cloud & DevOps (experiencia práctica)**

![Azure](https://img.shields.io/badge/Azure-0089D6?style=flat-square&logo=microsoft-azure&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)

**Bases de datos**

![SQL Server](https://img.shields.io/badge/SQL_Server-CC2927?style=flat-square&logo=microsoft-sql-server&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat-square&logo=postgresql&logoColor=white)
![Oracle](https://img.shields.io/badge/Oracle-F80000?style=flat-square&logo=oracle&logoColor=white)
![Cosmos DB](https://img.shields.io/badge/Cosmos_DB-0078D4?style=flat-square&logo=microsoft-azure&logoColor=white)

**IA & Automatización**

![GitHub Copilot](https://img.shields.io/badge/GitHub_Copilot-000000?style=flat-square&logo=github&logoColor=white)
![Claude](https://img.shields.io/badge/Claude-D97757?style=flat-square&logo=anthropic&logoColor=white)
![Make](https://img.shields.io/badge/Make-6D00CC?style=flat-square&logo=make&logoColor=white)
![N8N](https://img.shields.io/badge/N8N-EA4B71?style=flat-square&logo=n8n&logoColor=white)
![Power BI](https://img.shields.io/badge/Power_BI-F2C811?style=flat-square&logo=powerbi&logoColor=black)

---

## Formación

- 🎓 **Ingeniería de Sistemas** · Corporación Unificada Nacional *(2023–2025)*
- 📜 **Tecnólogo en Análisis y Desarrollo de Sistemas** · SENA *(2020–2022)*
- 📜 **Asistente en Desarrollo de Software** · CESDE *(2018–2020)*

---

## Actualmente

- 🔍 Explorando oportunidades en desarrollo fullstack y proyectos independientes
- 📚 Profundizando en arquitecturas de microservicios y automatización con N8N y Make
- ☁️ Explorando AWS como complemento a mi experiencia práctica en Azure
- 🌱 Mejorando mi inglés técnico activamente

---

<div align="center">

*"El mejor código es el que resuelve un problema real para una persona real."*

</div>
